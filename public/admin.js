...
function updateDeviceList(){
  ...
  for(let id in devices){
    const li = document.createElement('li'); li.textContent = `${id} (${devices[id].model})`; ul.appendChild(li);
...
function addLog(msg){
  ...
  const li = document.createElement('li'); li.textContent=`[${new Date().toLocaleTimeString()}] ${msg}`; ul.appendChild(li); ul.scrollTop = ul.scrollHeight;
}
