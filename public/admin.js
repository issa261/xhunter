...
// إعداد الخريطة
const map = L.map('map').setView([15.3694, 44.1910], 6); // اليمن كمركز أولي
...
socket.on('getLocation', data => {
    if(devices[data.id]){
        ...
        deviceMarkers[data.id] = L.marker([lat, lon]).addTo(map)
            .bindPopup(`${data.id} (${devices[data.id].model})`).openPopup();
    }
});
